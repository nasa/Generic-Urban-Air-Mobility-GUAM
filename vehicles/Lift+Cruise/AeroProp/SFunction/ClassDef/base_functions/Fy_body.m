function [y, y_x, y_u] = Fy_body(x,u,w,aero_coefs_pp,ders)
%
% Fb = Rx(gam)'*Fl
%
% Fy_b = -Fz_l*sin(gam)
...
if ders 
  ...
  Fy_x1 = Fy_Fzl.*Fzl_x(:,1);
  Fy_x2 = Fy_Fzl.*Fzl_x(:,2);
  Fy_x3 = Fy_Fzl.*Fzl_x(:,3);
  Fy_x4 = Fy_Fzl.*Fzl_x(:,4);
  Fy_x5 = Fy_Fzl.*Fzl_x(:,5);
  Fy_x6 = Fy_Fzl.*Fzl_x(:,6);

  %% take the derivates with respect to the inputs
  Fy_u1 = Fy_Fzl.*Fzl_u(:,1);
  Fy_u2 = Fy_Fzl.*Fzl_u(:,2);
  Fy_u3 = Fy_Fzl.*Fzl_u(:,3);
  Fy_u4 = Fy_Fzl.*Fzl_u(:,4);

  % x = [ u      v      w      p      q      r ]
  y_x = [ Fy_x1  Fy_x2  Fy_x3  Fy_x4  Fy_x5  Fy_x6 ];

  % u = [ T      To     del    i ]
  y_u = [ Fy_u1  Fy_u2  Fy_u3  Fy_u4];
else 
  y_x = [];
  y_u = [];
end
