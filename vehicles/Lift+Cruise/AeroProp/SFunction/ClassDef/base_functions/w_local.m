function [y, y_x, y_u] = w_local(x,u,w,ders)
%
% [u; v; w]_l = Ry(i)*Rx(gam).*([u; v; w] + [p; q;r] X [bx; by; bz])
%
% w_l = sin(iw)*(u + q*z - r*y) + cos(gam)*cos(iw)*(w + p*y - q*x) - cos(iw)*sin(gam)*(v - p*z + r*x)
%
% Inputs: 
%
...
if ders
  ...
  y_x = [ s_i...                               % u
         -s_gam.*c_i...                        % v
          c_gam.*c_i...                        % w
          c_gam.*c_i.*by + s_gam.*c_i.*bz...   % p
          sin(i).*bz - cos(gam).*cos(i).*bx... % q
         -sin(i).*by - sin(gam).*cos(i).*bx ]; % r

  % u = [ T           To         del         i ]
  y_u = [ zeros(N,1)  zeros(N,1) zeros(N,1)  cos(i).*(uu+q.*bz-r.*by)-cos(gam).*sin(i).*(ww+p.*by-q.*bx)+sin(gam).*sin(i).*(vv-p.*bz+r.*bx) ];
else
  y_x = zeros(N,6);
  y_u = zeros(N,4);
end
